...
const insterItem = (user_id, product_id, num) => {
    ...
    return new Promise((resolve, reject) => {
        db.query(sql, [user_id, product_id, num], (error, res) => {
            if (error) {
                reject()
            } else {
                resolve({
...
const updateItem = (product_id, obj) => {
    const sql = obj.type === 'num' ? `update cart set num = '${obj.num}' where product_id = '${product_id}'` : `update cart set active = '${obj.active}' where product_id = '${product_id}'`
    return new Promise((resolve, reject) => {
        db.query(sql, [], (error, res) => {
            if (error) {
                reject()
...
const updateAll = (type) => {
    const sql = `update cart set active = '${type}'`
    return new Promise((resolve, reject) => {
        db.query(sql, [], (error, res) => {
            if (error) {
                reject()
